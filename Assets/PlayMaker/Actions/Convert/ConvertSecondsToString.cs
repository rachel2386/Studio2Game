...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [Tooltip("Converts Seconds to a String value representing the totalTimeInSeconds.")]
	public class ConvertSecondsToString : FsmStateAction
	{
		...
		[RequiredField]
		[UIHint(UIHint.Variable)]
		[Tooltip("A string variable to store the totalTimeInSeconds value.")]
		public FsmString stringVariable;
        
		[RequiredField]
		[Tooltip("Format. 0 for days, 1 is for hours, 2 for minutes, 3 for seconds and 4 for milliseconds. 5 for total days, 6 for total hours, 7 for total minutes, 8 for total seconds, 9 for total milliseconds, 10 for two digits milliseconds. so {2:D2} would just show the seconds of the current totalTimeInSeconds, NOT the grand total number of seconds, the grand total of seconds would be {8:F0}")]
