...
//	Add caustics
		...
		//	This projects caustics also on the undersurface of the water – might be nice tho
			float causticsMask = 1; //cCancel; //(wPos.y > _Lux_UnderWaterWaterSurfacePos - 0.01) ? 0 : 1;

			causticsMask *= dirLightCancellation * s.Alpha.x;
...
//	Depth along the y axis
	...
	//	Fog lighting
		float3 fogLighting = _Lux_UnderWaterFogColor * ( _Lux_UnderWaterSunColor.rgb);
		float3 worldViewDir = viewDir; //normalize(_WorldSpaceCameraPos.xyz - IN.worldPos);
		float fCos = saturate( dot( _WorldSpaceLightPos0.xyz, -worldViewDir ) );
		float viewScatter = fCos * fCos * _Lux_UnderWaterUnderwaterScatteringPower;
		fogLighting *= viewScatter * _Lux_UnderWaterUnderwaterScatteringIntensity + 1.0f;
	//	Add ambient lighting to fog
		fogLighting += _Lux_UnderWaterFogColor * _Lux_UnderWaterAmbientSkyLight.rgb; // gi.indirect.diffuse; //
	//  Light cancellation on fog realtive to camera below water surface
		...
		c.rgb = lerp(c.rgb, fogLighting, fogDensity);
